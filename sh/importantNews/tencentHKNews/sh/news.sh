...
curl "http://finance.qq.com/l/hk/ggdt/index.htm" -o index.html

#url&&name
awk -F '<a target="_blank" href="' 'NF>1{print $2}' $index|awk -F '/a>' 'NF>1{printf $1}'|iconv -f gbk -t utf-8>$urlnameTemp
echo $(cat $urlnameTemp)|sed -e 's/"//g'|sed -e 's/</\n/g'|sed -e 's/>/\n/g'>$urlname

#url
awk 'NR%2' $urlname|sed -e 's/ /#/g'>$url

#name
awk '!(NR%2)' $urlname|sed -e 's/*//g;s/ /#/g'>$name

#time
awk -F '<span class="pub_time">' 'NF>1{print $2}' ./index.html|awk -F '/span>' 'NF>1{printf $1}'>$timeTemp
catTime=$(cat $timeTemp)
