...
curl "http://roll.finance.sina.com.cn/finance/gncj/gncj/index.shtml" -o index.html

#time
awk -F '</a><span>' 'NF>1{print $2}' $index|awk -F '</span></li>' 'NF>1{printf $1}'|iconv -f gbk -t utf-8|sed -e 's/(//g;s/)/\n/g'|sed -e 's/月/-/g;s/日//g'|sed -e "s/^/$year-/g;s/ /#/g">$time

#name
cat $index |grep '<li><a href="http://finance.sina.com.cn/china'|awk -F 'target="_blank">' 'NF>1{print $2}' |awk -F '/a>' 'NF>1{printf $1}'|iconv -f gbk -t utf-8|sed -e 's/*//g;s/</\n/g;s/ /#/g'>$name

#url
cat $index | grep '<li><a href="http://finance.sina.com.cn/china'|awk -F '<li><a href="' 'NF>1{print $2}' |awk -F ' target' 'NF>1{printf $1}'|sed -e 's/"/\n/g;s/ /#/g'>$url
