...
curl "http://finance.qq.com/gdyw.htm" -o index.html

#url&&name
awk -F '<em class="f20 l26"><a target="_blank" href="' 'NF>1{print $2}' $index|awk -F '/a>' 'NF>1{printf $1}'|sed -e 's/"//g'|sed -e 's/>/\n/g'|sed -e 's/</\n/g'|iconv -f gbk -t utf-8>$urlname

#url
echo "url"
awk 'NR%2' $urlname|sed -e "s/^/http:\/\/finance.qq.com/g">$url
...
#time
echo "time"
awk -F '<p class="time l22">' 'NF>1{print $2}' ./index.html | iconv -f gbk -t utf-8 | awk -F '/p>' 'NF>1{printf $1}' | sed -e 's/</\n/g'>$timeTemp
cat $timeTemp |sed -e 's/月/-/g;s/日//g' |sed -e "s/^/$year-/g;s/ /#/g">$time
#cat $timeTemp |sed -e "s/^/$year-/g"|sed 's/ /#/g'>$time
...
for i in "${!aName[@]}";
do
strcont=$(cat ../html/$i.html|tr -d '\n' |awk -F '<div id="Cnt-Main-Article-QQ" bossZone="content">' 'NF>1{print $2}'|awk -F '<div class="about-read">' 'NF>1{print $1}'| sed -e 's/<[^<]*>//g'|iconv -f gbk -t utf-8)
