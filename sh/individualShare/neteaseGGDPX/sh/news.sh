...
curl "http://money.163.com/special/00253K1N/ggdpx.html" -o index.html
#cat $index | iconv -f gbk -t utf-8>indexTemp.html
#mv indexTemp.html $index

#time
awk -F '<span class="time">' 'NF>1{print $2}' $index|awk -F '</span>' 'NF>1{print $1}'|sed -e 's/*//g;s/ /#/g'>$time
count=$(cat $time|wc -l)
...
#name
awk -F 'html">' 'NF>1{print $2}' $urlname|awk -F '</a>' 'NF>1{print $1}'|sed -e 's/ /#/g'|iconv -f gbk -t utf-8>$nameTemp
head -n$count $nameTemp>$name
#cat name.txt | iconv -f gbk -t utf-8>$name
...
for i in "${!aName[@]}";
do
    strcont=$(cat ../html/$i.html|tr -d '\n' |awk -F '<div id="endText" class="end-text">' 'NF>1{print $2}'|awk -F '<div class="sharecommend-wrap clearfix">' 'NF>1{print $1}'| sed -e 's/<[^<]*>//g' | iconv -f gbk -t utf-8)
