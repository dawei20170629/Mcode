...
curl "http://roll.finance.sina.com.cn/finance/xg/xg_zxdt/index.shtml" -o index.html


#url&&name
cat $index |grep '<li><a href="http://finance.sina.com.cn'>$urlnameTemp


#url
awk -F '<a href="' 'NF>1{print $2}' $urlnameTemp|awk -F '" target=' 'NF>1{print $1}'|sed -e 's/ /#/g'>$url

#name
awk -F 'target="_blank">' 'NF>1{print $2}' $urlnameTemp|awk -F '</a>' 'NF>1{print $1}'|sed -e 's/*//g;s/ /#/g'| iconv -f gbk -t utf-8>$name

#time
awk -F '<span>' 'NF>1{print $2}' $urlnameTemp|awk -F ')</span>' 'NF>1{print $1}'|sed -e 's/(//g'|sed -e 's/ /#/g'|sed -e "s/^/$year-/g"| iconv -f gbk -t utf-8|sed -e 's/月/-/g;s/日//g'>$time
...
for i in "${!aName[@]}";
do
strcont=$(cat ../html/$i.html|tr -d '\n' |awk -F '<div class="finance_app_zqtg">' 'NF>1{print $2}'|awk -F '<p class="art_keywords">' 'NF>1{print $1}'| sed -e 's/<[^<]*>//g'| iconv -f gbk -t utf-8)
