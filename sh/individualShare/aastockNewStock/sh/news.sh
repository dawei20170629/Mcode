...
curl "http://www.aastocks.com/sc/ipo/ipoNews.aspx" -o index.html

#time
cat $index|grep "$day"|sed -e 's/  //g'|sed -e 's/<\/span>//g'|sed -e 's/ /#/g'>$time
count=$(cat $time|wc -l)

#url
cat $index | grep 'class="font15a"'|sed -e 's/ //g'>$urlnameTemp
cat $index | grep 'class="n_Title"'|sed -e 's/ //g'>>$urlnameTemp
head -n$count $urlnameTemp>$urlTemp
awk -F "HK6','" '{print $2}' $urlTemp | awk -F ")" '{printf $1}' | sed -e "s/'/\n/g" | sed -e 's/^/http:\/\/www.aastocks.com\/sc\/stocks\/news\/aafn-news\//g' | sed -e 's/$/\/1/'|sed -e 's/ /#/g'>$url

#name
cat $index|grep '《新股消息》'|sed -e 's/ //g'|sed -e 's/ /#/g'>$nameTemp
...
for i in "${!aName[@]}";
do
    strcont=$(cat ../html/$i.html|tr -d '\n' |awk -F '<span id="lblContent" class="mcFont2' 'NF>1{print $2}'|awk -F '<div id="divsubnews">' 'NF>1{print $1}'| sed -e 's/<[^<]*>//g')
