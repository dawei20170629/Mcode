...
curl "http://stock.stockstar.com/list/company.htm" -o index.html

#url&name
cat $index | grep '<li><span>' | sed -e 's/<\/li>/\n/g' |sed -e 's/<li>/\n/g'|sed -e 's/ /#/g'>$urlname

#url
awk -F 'href="' 'NF>1{print $2}' $urlname|awk -F '">' 'NF>1{print $1}'|sed -e 's/ /#/g'>$url

#time
awk -F '<span>' 'NF>1{print $2}' $urlname|awk -F '</span>' 'NF>1{print $1}'|sed -e 's/ /#/g'>$time

#name
awk -F 'shtml">' 'NF>1{print $2}' $urlname|awk -F '</a>' 'NF>1{print $1}'|sed -e 's/*//g;s/ /#/g'|iconv -f gbk -t utf-8>$name
...
for i in "${!aName[@]}";
do
    strcont=$(cat ../html/$i.html|tr -d '\n' |awk -F '<div class="article" id="container-article">' 'NF>1{print $2}'|awk -F '<p style=' 'NF>1{print $1}'| sed -e 's/<[^<]*>//g'|iconv -f gbk -t utf-8)
