...
curl "http://xueqiu.com/today/all" -o index.html

#url&name&time
cat index.html | sed -e 's/<div class="list_item_rb">/\n/g' | grep '<div class="list_item_tit">'>$urlname

#time
day=$(date +'%m-%d')
year=$(date +'%Y')
awk -F '<span class="item_time">' 'NF>1{print $2}' $urlname|awk -F '/span>' 'NF>1{printf $1}'|sed -e 's/</\n/g'|sed -e 's/今天/'$day'/g'| sed -e 's/^/'$year'-/g'|sed -e 's/ /#/g'>$time
lineNum=$(cat $time|wc -l)

#url
awk -F '<div class="list_item_tit"><a href="' 'NF>1{print $2}' $urlname|awk -F ' target' 'NF>1{printf $1}'|sed -e 's/"/\n/g'|sed -e 's/^/http:\/\/xueqiu.com/g'|sed -e 's/ /#/g'>$url

#name
awk -F 'arget="_blank">' 'NF>1{print $2}' $urlname|awk -F '/a>' 'NF>1{printf $1}'|sed -e 's/</\n/g'|sed -e 's/*//g;s/ /#/g'>$name
...
for i in "${!aName[@]}";
do
strcont=$(cat ../html/$i.html|tr -d '\n' |awk -F 'class="single-description"' 'NF>1{print $2}'|awk -F 'class="donators">' 'NF>1{print $1}'| sed -e 's/<[^<]*>//g')
