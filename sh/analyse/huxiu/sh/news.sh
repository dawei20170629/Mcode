...
curl "http://www.huxiu.com/" -o index.html

#time
awk -F '<time>' 'NF>1{print $2}' $index|awk -F '/time>' 'NF>1{printf $1}'|sed -e 's/</\n/g'|sed -e 's/ /#/g'>$time
lineNum=$(cat $time|wc -l)

#url&&name
cat index.html | grep '<h2><a href="/article'>$urlnameTemp
cat index.html | grep '<h4><a href="/article'>>$urlnameTemp
head -n$lineNum $urlnameTemp>$urlname

#url
awk -F '<a href="' 'NF>1{print $2}' $urlname|awk -F ' target' 'NF>1{printf $1}'|sed -e 's/"/\n/g'>$urlTemp
sed -e 's/^/http:\/\/www.huxiu.com/g' $urlTemp|sed -e 's/ /#/g'>$url

#name
sed -e 's/<h4>//g' $urlname |sed -e 's/<h2>//g' | awk -F '>' 'NF>1{print $2}' |awk -F '/a' 'NF>1{printf $1}'|sed -e 's/</\n/g'|sed -e 's/*//g;s/ /#/g'>$name
...
for i in "${!aName[@]}";
do
strcont=$(cat ../html/$i.html|tr -d '\n' |awk -F '<div class="neirong-box" id="neirong_box">' 'NF>1{print $2}'|awk -F '<div class="neirong-shouquan">' 'NF>1{print $1}'| sed -e 's/<[^<]*>//g')
